...
DATABASE_URL = (
    ...
    or os.getenv("DATABASE_URL".lower())
)
if not DATABASE_URL:
...
app = FastAPI(title="MedAI v2.3 (import)", version="2.3.0")
...
def detect_category(text: str) -> str:
    """
    Jednoduchá heuristika na rozpoznanie kategórie podľa obsahu textu.
    """
    t = (text or "").lower()
    if any(x in t for x in ["crp", "hb", "leu", "na+", "k+", "mg/l", "mmol", "gluk"]):
        return "LAB"
    if any(x in t for x in ["ekg", "fibril", "tachy", "brady"]):
        return "EKG"
    if any(x in t for x in ["rtg", "rentgen", "röntgen", "infiltrát", "pneumon"]):
        return "RTG"
    if any(x in t for x in ["podaná", "podan", "ceftriax", "amoxicil", "lieč", "infuz", "antibio"]):
        return "THERAPY"
    return "NOTE"
...
async def import_data(
    ...
    db.add(r); db.commit(); db.refresh(r)
    return {"status": "imported", "patient_uid": patient_uid, "category": cat, "record_id": r.id}
...
<title>MedAI Dashboard 2.3 (import)</title>
<style>
...
<body>
  ...
  <div class="wrap">
    ...
    <div class="card">
      ...
      <textarea id="imp_text" class="inp" rows="4" placeholder="Sem vlož text (napr. lab/rtg/ekg popis) — alebo použi súbor nižšie."></textarea>
...
function renderTable(items){
  if(!items || !items.length){
    $('list').innerHTML = '<small>Žiadne záznamy.</small>'; return;
  }
...
async function doImport(){
  ...
  try{
    ...
    if(!res.ok){ const t = await res.text(); throw new Error(t); }
    const data = await res.json();
    $('imp_msg').innerHTML = '<span class="ok">Import OK ('+data.category+')</span>';
    if($('uid').value.trim()){ await loadRecords(); }
  }catch(e){
