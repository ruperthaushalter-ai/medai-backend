...
# API key môže prísť v hlavičke x-api-key alebo v URL ?api_key=
def require_api_key(
...
class RecordIn(BaseModel):
    timestamp: Optional[str] = None   # ISO 8601, inak sa doplní now()
    category: Optional[str] = None    # ak nie je, zistí sa heuristikou
    content: str
...
def detect_category(text: str) -> str:
    ...
    if any(x in t for x in ["crp", "hb", "leu", "mg/l", "mmol", "gluk"]):
        return "LAB"
    if any(x in t for x in ["ekg", "fibril", "tachy", "brady", "interval", "qrs", "st "]):
        return "EKG"
    if any(x in t for x in ["rtg", "rentgen", "infiltr", "pneumon", "tieňovanie"]):
        return "RTG"
    if any(x in t for x in ["podan", "lieč", "infuz", "antibio", "ceftriax", "amoxicil", "heparin"]):
        return "THERAPY"
    if any(x in t for x in ["tlak", "puls", "dych", "teplota", "satur"]):
        return "VITALS"
    return "NOTE"
...
def list_patients(db: Session = Depends(get_db)):
    pts: List[Patient] = db.query(Patient).order_by(Patient.created_at.desc()).all()
    return [
        {
...
.badge{display:inline-block;background:#e8f7f0;color:#0a7f58;border-radius:999px;padding:4px 10px}
...
const api = (path, opt={})=>{
  ...
  // posielame v hlavičke; backend akceptuje aj ?api_key=
